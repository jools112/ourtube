...
const VideoSummaryLoaded = (props) => {
  ...
  const _videoDuration = videoData.contentDetails.duration
    ...
    .join(':')
  const videoDuration = _videoDuration.includes(':')
    ? _videoDuration
    : '00:' + _videoDuration
  const viewCount = formatViewers(Number(videoData.statistics.viewCount))
  const _likePct = parseFloat(
    (
      (100 * Number(videoData.statistics.likeCount)) /
      (Number(videoData.statistics.likeCount) +
        Number(videoData.statistics.dislikeCount))
    ).toFixed(1)
  )
  ...
  if (props.mini != 'true') {
    return (
      <SoftBox
        title={videoTitle.slice(0, 40)}
        content={
          <div className="videoSummaryContainer" onClick={clickHandler}>
            <img
              src={thumbnail.url}
              alt="Video Thumbnail"
              className="videoSummaryThumbnail"
            />
    ...
    return (
      <div className="videoSummaryContainerSmall" onClick={clickHandler}>
